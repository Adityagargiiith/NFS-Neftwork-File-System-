...
int copydirnm(char *src, char *dest, int client_socket)
{
    ...
    if (ans.s2s_port == ans1.s2s_port && strcmp(ans.ss_ip, ans1.ss_ip) == 0)
    {
        ...
        if (status == SUCCESS)
        {
            ...
            insert_into_tree_new(new_path, 1, copy1.ss_ip, copy1.ss_port, copy1.client_port, copy1.s2s_port, 1);
        }
        else
        {
    ...
    if (status == SUCCESS && status1 == SUCCESS)
    {
        ...
        strcat(new_path, temp);
        new_path[strlen(new_path)] = '\0';
        insert_into_tree_new(new_path, 1, copy2.ss_ip, copy2.ss_port, copy2.client_port, copy2.s2s_port, 1);
        return SUCCESS;
    }
    else
    {
